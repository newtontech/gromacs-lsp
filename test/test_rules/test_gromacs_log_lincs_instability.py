"""Golden/fixture contract for the gromacs.log.lincs_instability rule (issue #20).

These tests pin the OpenQC rule surface for LINCS instability detection:

* the invalid fixture must emit exactly the ``gromacs.log.lincs_instability``
  diagnostic at ``error`` severity, with a stable range and fix hint;
* the valid fixture must emit no ``gromacs.log.lincs_instability`` diagnostic;
...
def test_invalid_fixture_matches_golden() -> None:
    fixture = LOG_FIXTURES / "lincs_instability.log"
    golden_path = Path(__file__).resolve().parents[1] / "fixtures" / "rules" / "log_lincs_instability.json"
...
def test_log_tool_operation(tmp_path: Path) -> None:
    """The gromacs-lsp-tool log operation must surface LINCS instability."""
    log_file = tmp_path / "test.log"
    log_file.write_text("Step 100, time 0.2 (ps)  LINCS warning\n", encoding="utf-8")
