"""Golden/fixture contract for the gromacs.log.fatal_error rule (issue #22).

These tests pin the OpenQC rule surface for GROMACS fatal error detection:

* the invalid fixture must emit exactly the ``gromacs.log.fatal_error``
  diagnostic at ``error`` severity, with a stable range and fix hint;
* the valid fixture must emit no ``gromacs.log.fatal_error`` diagnostic;
...
def test_invalid_fixture_matches_golden() -> None:
    fixture = LOG_FIXTURES / "fatal_error.log"
    golden_path = Path(__file__).resolve().parents[1] / "fixtures" / "rules" / "log_fatal_error.json"
...
def test_log_tool_operation(tmp_path: Path) -> None:
    """The gromacs-lsp-tool log operation must surface fatal errors."""
    log_file = tmp_path / "test.log"
    log_file.write_text("Fatal error: something went wrong\n", encoding="utf-8")
