"""Golden/fixture contract for the gromacs.topology.molecule_count_mismatch rule (issue #18).

These tests pin the OpenQC rule surface for ``[ molecules ]`` entries that
reference a molecule type with no matching ``[ moleculetype ]`` definition:

* the invalid fixture must emit exactly the
  ``gromacs.topology.molecule_count_mismatch`` diagnostic at ``error``
  severity, with a stable range and fix hint;
* the valid fixture (every ``[ molecules ]`` name has a matching
  ``[ moleculetype ]`` block) must emit no
  ``gromacs.topology.molecule_count_mismatch`` diagnostic;
* the rule manifest under ``rules/diagnostics.yaml`` must export the rule with
  the expected severity and source metadata;
* ``gromacs-lsp-tool explain --json gromacs.topology.molecule_count_mismatch``
  must surface the same rule id for OpenQC consumers.
"""
...
def test_invalid_fixture_matches_golden() -> None:
    fixture = FIXTURES / "molecule_count_mismatch.top"
    golden_path = FIXTURES / "molecule_count_mismatch.json"
    ...
    for actual, expected in zip(items, golden["diagnostics"]):
        ...
        assert actual["category"] == "type/value"
...
def test_valid_fixture_does_not_trigger() -> None:
    fixture = FIXTURES / "valid_topology_molecules.top"
    diagnostics = [d for d in analyze_file(fixture) if d.rule_id == RULE_ID]
    assert diagnostics == []
...
def test_molecule_count_mismatch_severity_is_error(tmp_path: Path) -> None:
    """Promotion check: undefined [ molecules ] references must be blocking errors, not warnings."""
    fixture = tmp_path / "mismatch.top"
    fixture.write_text(
        "[ moleculetype ]\n"
        "SOL     3\n\n"
        "[ molecules ]\n"
