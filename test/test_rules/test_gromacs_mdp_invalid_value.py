"""Golden/fixture contract for the gromacs.mdp.invalid_value rule (issue #16).

These tests pin the OpenQC rule surface for invalid MDP values:

* the invalid fixture must emit exactly the ``gromacs.mdp.invalid_value``
  diagnostic at ``error`` severity, with a stable range and fix hint;
* the valid fixture must emit no ``gromacs.mdp.invalid_value`` diagnostic;
* the rule manifest under ``rules/diagnostics.yaml`` must export the rule with
  the expected severity and source metadata;
* ``gromacs-lsp-tool explain --json gromacs.mdp.invalid_value`` must surface
...
def test_invalid_fixture_matches_golden() -> None:
    fixture = FIXTURES / "invalid_mdp_value.mdp"
    golden_path = FIXTURES / "invalid_mdp_value.json"
...
def test_invalid_value_severity_is_error(tmp_path: Path) -> None:
    """Promotion check: invalid MDP values must be blocking errors, not warnings."""
    fixture = tmp_path / "badval.mdp"
    fixture.write_text(
        "integrator = warp9\nnsteps = 1\ndt = 0.002\n", encoding="utf-8"
    )
    diagnostics = [d for d in analyze_file(fixture) if d.rule_id == RULE_ID]
