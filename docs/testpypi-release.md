...
```bash
git tag testpypi-v0.0.4rc1
git push origin testpypi-v0.0.4rc1
```
...
```bash
python -m venv /tmp/gromacs-lsp-testpypi
/tmp/gromacs-lsp-testpypi/bin/python -m pip install --upgrade pip
/tmp/gromacs-lsp-testpypi/bin/pip install \
  --index-url https://test.pypi.org/simple/ \
  --extra-index-url https://pypi.org/simple/ \
  gromacs-lsp==0.0.4rc1
```
