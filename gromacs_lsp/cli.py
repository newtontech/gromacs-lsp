...
def lsp_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="gromacs-lsp")
    parser.add_argument(
        "--stdio", action="store_true", help="start the LSP server on stdio"
    )
    args = parser.parse_args(argv)
    if not args.stdio:
        parser.error("only --stdio is currently supported")
    print(
        "gromacs-lsp scaffold: full JSON-RPC LSP is tracked in roadmap issues",
        file=sys.stderr,
    )
    return 0
...
def lint_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="gromacs-lint")
    ...
    if args.json:
        print(
            json.dumps(
                [item.to_json() for item in diagnostics], indent=2, sort_keys=True
            )
        )
    else:
...
def fmt_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="gromacs-fmt")
...
def test_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="gromacs-test")
